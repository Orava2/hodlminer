...
static pthread_mutex_t scratchpad_lock;
uint32_t merkle = 0;
...
//		   result ? "(yay!!!)" : "(booooo)");
	if (result){
			...
			applog(LOG_INFO, "%saccepted%s %lu/%lu (%.2f%%), %s%s%s h/s (%ldh/%.1fs), time avg %d samples %s%s%s h/s",
			...
			applog(LOG_INFO, "%srejected%s %lu/%lu (%.2f%%), %s%s%s h/s (%ldh/%.1fs), time avg %d samples %s%s%s h/s",
...
static bool submit_upstream_work(CURL *curl, struct work *work)
{
	...
	if (have_stratum) {
		...
		char ntimestr[9], noncestr[9], nstartlocstr[9], nfinalcalcstr[9], *xnonce2str, *req;
		...
		sprintf(req,
			"{\"method\": \"mining.submit\", \"params\": [\"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\"], \"id\":4}",
			rpc_user, work->job_id, xnonce2str, ntimestr, noncestr, nstartlocstr, nfinalcalcstr);
		free(xnonce2str);
...
if (SetConsoleMode(hOut, dwMode)){
	...
	pthread_mutex_init(&stratum.work_lock, NULL);
        pthread_mutex_init(&scratchpad_lock, NULL);
